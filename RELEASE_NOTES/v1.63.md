# v1.63 — User-Configurable CPU Offload Setting (Issue #21)

## Summary
- **Fixed**: Addressed [Issue #21](https://github.com/ussoewwin/ComfyUI-QwenImageLoraLoader/issues/21) – CPU offload setting was hardcoded to `"auto"`, causing unnecessary slowdowns
...
wrapped_model = ComfyQwenImageWrapper(
    ...
    None,
    {},
    cpu_offload,  # ← User-selected value
    4.0,
)
```
...
def IS_CHANGED(s, model, lora_name, lora_strength, cpu_offload, *args, **kwargs):
    ...
    m.update(cpu_offload.encode())  # ← Added
    return m.digest().hex()
```
...
- ✅ **User Control**: Users can now explicitly control CPU offload behavior
- ✅ **Performance**: Default `"disable"` setting provides better performance for users with sufficient VRAM
- ✅ **Flexibility**: Users with low VRAM can still use `"auto"` or `"enable"` modes
- ✅ **Visibility**: Comprehensive logging helps users verify CPU offload settings
- ✅ **Cache Management**: Proper cache invalidation when CPU offload setting changes

## Backward Compatibility

- ⚠️ **Breaking Change**: Existing workflows need to be updated
...
### Verified Scenarios
- ✅ New wrapper creation with different `cpu_offload` settings
- ✅ Existing wrapper setting update
- ✅ Cache invalidation when `cpu_offload` setting changes
- ✅ Logging output for all scenarios
- ✅ Both `NunchakuQwenImageLoraLoader` and `NunchakuQwenImageLoraStack` nodes
...
📦 Current CPU offload setting: 'auto'
🔄 Updating CPU offload setting from 'auto' to 'disable'
```
