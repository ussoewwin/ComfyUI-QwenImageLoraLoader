...
console.log("★★★ z_qwen_lora_dynamic_v3.js: Qwen Image LoRA Stack V3 ★★★");
...
app.registerExtension({
    ...
    nodeCreated(node) {
        ...
        // Hide lora_count widget in UI (but keep it for backend)
        ...
        const initCache = () => {
            ...
            if (cpuOffloadWidget) node.cachedCpuOffload = cpuOffloadWidget;
            ...
            if (!w) {
                ...
                w = node.addWidget("combo", name, "1", (v) => {
                    ...
                    if (!isNaN(num)) {
                        ...
                        if (node.cachedLoraCount) node.cachedLoraCount.value = num;
                        node.updateLoraSlots();
                    }
                }, { values });
            }
            w.value = node.properties["visibleLoraCount"].toString();
            if (node.cachedLoraCount) node.cachedLoraCount.value = node.properties["visibleLoraCount"];
            return w;
        };

        const ensureToggleAllWidget = () => {
            if (!node.cachedToggleAll) return null;
            if (!node.cachedToggleAll.origCallback) node.cachedToggleAll.origCallback = node.cachedToggleAll.callback;
            node.cachedToggleAll.callback = (value) => {
                if (node.cachedToggleAll.origCallback) node.cachedToggleAll.origCallback(value);
                const count = parseInt(node.properties["visibleLoraCount"] || 1);
                for (let i = 1; i <= count; i++) {
                    const pair = node.cachedWidgets[i];
                    if (pair && pair[0]) pair[0].value = value;
                }
            };
        ...
        node.updateLoraSlots = function () {
            ...
            if (toggleAllWidget) this.widgets.push(toggleAllWidget);
            ...
            for (let i = 1; i <= count; i++) {
                ...
                if (pair && pair.length >= 3) {
                    ...
                    if (wName.computeSize) delete wName.computeSize;
                    if (wStrength.computeSize) delete wStrength.computeSize;
                    this.widgets.push(wEnabled, wName, wStrength);
                }
            }
