...
console.log("★★★ zimageturbo_lora_dynamic.js: Z-Image-Turbo LoRA Stack V2 ★★★");

const HIDDEN_TAG = "tschide";

app.registerExtension({
    name: "nunchaku.zimageturbo_lora_dynamic_combo_final_restore",
    
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "NunchakuZImageTurboLoraStackV2") {
            nodeType["@visibleLoraCount"] = { type: "number", default: 1, min: 1, max: 10, step: 1 };
        }
    },

        nodeCreated(node) {
        if (node.comfyClass !== "NunchakuZImageTurboLoraStackV2") return;
