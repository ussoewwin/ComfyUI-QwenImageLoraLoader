# Krea2 Depth ControlNet LoRA — Complete Technical Guide

This document is the full technical explanation of **Krea2 depth ControlNet LoRA** support added after baseline commit **`ddb37ff19ee358c6fd9ec777bd4334c72b18d229`**.

It covers:

1. Every **new or modified** source file
2. **Full text** of added/changed code (not summaries)
3. **Meaning** of each block
4. End-to-end **architecture**, routing, and workflow usage

**Scope:** Depth-only Krea2 control LoRA today. The loader reads from the ComfyUI **`controlnet`** folder; application runs through **`NunchakuQwenImageDiffsynthControlnet`** when the base model is Krea2 (`SingleStreamDiT` / `comfy.ldm.krea2`).
