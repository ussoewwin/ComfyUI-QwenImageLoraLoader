# Z-Image / SVDQ: `.dtype` on `NoneType` — ComfyUI deferred `Linear`, `pop` default evaluation, and mitigation in this node pack (detailed)

This document explains, at similar depth to other `md/` notes, a crash that can occur when loading Nunchaku-quantized Z-Image (Lumina2 / NextDiT) via **ComfyUI-nunchaku**: **what the error is, why it happens, the underlying causes, why ComfyUI-QwenImageLoraLoader absorbs the fix, which files change, and what the added code does**.
