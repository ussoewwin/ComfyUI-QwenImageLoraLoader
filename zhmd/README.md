...
> 最新版本: [v2.5.8 发行说明](https://github.com/ussoewwin/ComfyUI-QwenImageLoraLoader/releases/tag/v2.5.8)
...
6. **Krea2 控制支持**（depth & openpose）：添加 `Krea2ControlNetLoraLoader`，选择你的
   ...
   - **OpenPose**（例如 `krea2_turbo_openpose_controlnet.safetensors`）：纯块 LoRA 路由，作用于
     `diffusion_model.blocks` 与 `diffusion_model.txtfusion`（layerwise/refiner），并将姿态图
...
**工作原理：**
- 256 个块 LoRA 补丁（rank 32）应用于全部 28 个 DiT 块以及 txtfusion 的 layerwise/refiner 块
- 姿态图经 VAE 编码后，以原生 Krea2 参考潜变量（`index_timestep_zero`）注入——这正是该控制
  LoRA 训练时所采用的条件通路
- `use_kv_cache` 在 t=0 时执行一次参考前向，预先计算每个块的 K/V，并在每个采样步将其作为
  额外 attention key 注入（序列中不携带逐步 ref token）

**互斥性：** Krea2 路由严格子分类（depth vs openpose），整个 Krea2 分支与 Qwen Image /
Z-Image / Nunchaku 路由完全隔离——使用这些路由的现有工作流不受任何影响。

*参考：ostris 的 `comfyui-krea2-ostris-edit` TextEncode 节点与 pysssss 的
`krea2_controlnet_pose.json` 工作流仅作参考；本仓库已独立实现相同的姿态控制效果。*
...
## 更新日志
  ...
  有关最新更新和详细发行说明（v2.4.0 起），请参阅 [changelogzh.md](changelogzh.md)。
